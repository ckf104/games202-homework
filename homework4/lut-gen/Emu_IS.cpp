...
Vec3f ImportanceSampleGGX(Vec2f Xi, Vec3f N, float roughness) {
    ...
    float theta_m = std::atan(a * std::sqrt(Xi.x / (1 - Xi.x)));
    float phi_m = 2 * PI * Xi.y;

    //TODO: from spherical space to cartesian space - Bonus 1
    float sin_theta_m = std::sin(theta_m);
    float cos_theta_m = std::cos(theta_m);
    float sin_phi_m = std::sin(phi_m);
    float cos_phi_m = std::cos(phi_m);
 
    return Vec3f(sin_theta_m * cos_phi_m, sin_theta_m * sin_phi_m, cos_theta_m);
...
Vec3f IntegrateBRDF(Vec3f V, float roughness) {
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        float denom = NoV * NoH;
        if (denom > 0.0f) {
            float weight =  VoH * GeometrySmith(roughness, NoV, NoL) / (denom);        
            num += weight;
        }
