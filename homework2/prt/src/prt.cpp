            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    for(int l = 0; l <= SHOrder; ++l) {
                        for(int m = -l; m <= l; ++m) {
                            auto shv = (float)sh::EvalSH(l, m, dir.cast<double>().normalized());
                            auto area = CalcArea(x, y, width, height);
                            SHCoeffiecents[sh::GetIndex(l, m)] += Le * area * shv;
                        }
                    }
                }
            }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                else
                {
                    ...
                    auto hs = std::max(wi.dot(n), 0.0f);
                    float visibility = 1.0f - (float)scene->rayIntersect(Ray3f(v, wi)); 
                    return co * hs * visibility;
                }
            };
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            std::vector<std::vector<float>> bounceCoeffs(m_Bounce);
            for(int i = 0; i < m_Bounce; i++) {
                bounceCoeffs[i].resize(mesh->getVertexCount() * SHCoeffLength);
                for(int v = 0, maxv = mesh->getVertexCount(); v < maxv; v++) {
                    const Point3f pos = mesh->getVertexPositions().col(v);
                    const Normal3f normal = mesh->getVertexNormals().col(v);
                    ...
                    for (int t = 0; t < sample_side; t++) {
                      for (int p = 0; p < sample_side; p++) {
                        ...
                        const auto wi = Vector3f(dir.x(), dir.y(), dir.z());
                        
                        Intersection its;
                        bool visibility = scene->rayIntersect(Ray3f(pos, wi), its);
                        if (visibility) {
                            auto idx1 = (int)its.tri_index.x();
                            auto idx2 = (int)its.tri_index.y();
                            auto idx3 = (int)its.tri_index.z();
                            auto hs = std::max(wi.dot(normal), 0.0f);
                            if (i == 0) {
                                for(int j = 0; j < SHCoeffLength; j++) {
                                    bounceCoeffs[i][v * SHCoeffLength + j] += its.bary.x() * m_TransportSHCoeffs.col(idx1).coeff(j);
                                    bounceCoeffs[i][v * SHCoeffLength + j] += its.bary.y() * m_TransportSHCoeffs.col(idx2).coeff(j);
                                    bounceCoeffs[i][v * SHCoeffLength + j] += its.bary.z() * m_TransportSHCoeffs.col(idx3).coeff(j);
                                    bounceCoeffs[i][v * SHCoeffLength + j] *= co * hs;
                                }
                            } else {
                                for(int j = 0; j < SHCoeffLength; j++) {
                                    bounceCoeffs[i][v * SHCoeffLength + j] += its.bary.x() * bounceCoeffs[i - 1][idx1 * SHCoeffLength + j];
                                    bounceCoeffs[i][v * SHCoeffLength + j] += its.bary.y() * bounceCoeffs[i - 1][idx2 * SHCoeffLength + j];
                                    bounceCoeffs[i][v * SHCoeffLength + j] += its.bary.z() * bounceCoeffs[i - 1][idx3 * SHCoeffLength + j];
                                    bounceCoeffs[i][v * SHCoeffLength + j] *= co * hs;
                                }
                            }
                        }
                    ...
                    const float weight = 4.0f * M_PI / (sample_side * sample_side);
                    for(int j = 0; j < SHCoeffLength; j++) {
                        bounceCoeffs[i][v * SHCoeffLength + j] *= weight;
                    }
                }
            }
            for(int i = 0; i < m_Bounce; i++) {
                for(int v = 0, maxv = mesh->getVertexCount(); v < maxv; v++) {
                    for(int j = 0; j < SHCoeffLength; j++) {
                        m_TransportSHCoeffs.col(v).coeffRef(j) += bounceCoeffs[i][v * SHCoeffLength + j];
                    }
                }
            }
        }
