...
let numTransportType = 4;
let precomputeLTAllType = [];
...
async function GAMES202Main() {
	...
	// file parsing
	for (let t = 0; t < numTransportType; t++) {
		precomputeLTAllType[t] = [];
	for (let i = 0; i < envmap.length; i++) {
	
	let val = '';
	let typeName = transportTypeSuffix[t];
	await this.loadShaderFile(envmap[i] + `/transport_${typeName}.txt`).then(result => {
		val = result;
	});

	let preArray = val.split(/[(\r\n)\r\n' ']+/);
	precomputeLT[i] = precomputeLTAllType[t][i] = new Array(Number(preArray.length) - 2);
	...
	function createGUI() {
		...
		panelModel.add(guiParams, 'transportType', { 'Unshadow': 0, 'Shadow': 1, 'Shadow bounce 1': 2, 'Shadow bounce 2': 3}).name('Transport Type');
		panelModel.open();
	}
